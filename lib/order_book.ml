  ...
  | Some bb, Some ba ->
      if (bb.price > ba.price) && (bb.user <> ba.user) then
        ...
        if bb.quantity = ba.quantity then
          ...
          let b1, u1 =
            ( modify_book_order new_book bb bb.o_type bb.asset bb.price qdiff,
              modify_user_pending_orders new_users bb bb.o_type bb.asset bb.price qdiff )
          in
          let b2, u2 =
            (remove_from_book b1 ba, remove_from_user_pending u1 ba)
          in
          ...
          in
          let b2, u2 =
            ( modify_book_order b1 ba ba.o_type ba.asset ba.price qdiff,
            modify_user_pending_orders u1 ba ba.o_type ba.asset ba.price qdiff )
          in
          (b2, add_profit (add_profit u2 bb.user ~-(bb.price * qdiff)) ba.user (ba.price * qdiff))
